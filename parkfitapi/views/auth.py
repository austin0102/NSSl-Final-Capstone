...
from parkfitapi.models import Trainer, Athlete
...
def register_user(request):
    ...
    if email is not None and first_name is not None and last_name is not None and password is not None:
        try:
            ...
            new_user = User.objects.create_user(
                ...
                first_name=request.data['first_name'],
                last_name=request.data['last_name'],
                is_staff=is_staff  # Set is_staff based on the request data
            )

            if is_staff:
                # Create a trainer profile if is_staff is true
                trainer = Trainer.objects.create(
                    user=new_user
                )
            else:
                # Create an athlete profile if is_staff is false
                athlete = Athlete.objects.create(
                    user=new_user
                )
        except IntegrityError:
    ...
    return Response({'message': 'You must provide email, password, first_name, last_name, and is_staff'},
                    status=status.HTTP_400_BAD_REQUEST)
